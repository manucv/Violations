...
CREATE TABLE IF NOT EXISTS `violationsg`.`compra_saldo` (
  ...
  INDEX `fk_compra_saldo_parqueadero_punto_recarga1_idx` (`pun_rec_id` ASC),
...
-- -----------------------------------------------------
-- Table `violationsg`.`relacion_cliente`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `violationsg`.`relacion_cliente` (
  `rel_cli_id` INT NOT NULL AUTO_INCREMENT,
  `cli_id` INT NOT NULL,
  `cli_id_relacionado` INT NOT NULL,
  `rel_cli_hora` TIMESTAMP NULL,
  `rel_cli_tipo` VARCHAR(45) NULL,
  INDEX `fk_relacion_cliente_cliente1_idx` (`cli_id` ASC),
  INDEX `fk_relacion_cliente_cliente2_idx` (`cli_id_relacionado` ASC),
  PRIMARY KEY (`rel_cli_id`),
  CONSTRAINT `fk_relacion_cliente_cliente1`
    FOREIGN KEY (`cli_id`)
    REFERENCES `violationsg`.`cliente` (`cli_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_relacion_cliente_cliente2`
    FOREIGN KEY (`cli_id_relacionado`)
    REFERENCES `violationsg`.`cliente` (`cli_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
